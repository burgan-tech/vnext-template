---
description: Initialize the template into a concrete domain by replacing {domainName}
...
- Run `npm run setup <domain-name>` (which runs `setup.js`) to replace every
  `{domainName}` placeholder across the template files and folders.
...
tree already contains a concrete (non-`{domainName}`) domain.
